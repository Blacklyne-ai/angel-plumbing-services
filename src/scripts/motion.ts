...
if (!reduce && "IntersectionObserver" in window) {
  ...
  // Safety net: never leave content invisible. Force-reveal anything still
  // hidden after load, and reveal above-the-fold elements promptly.
  const failsafe = () => {
...
/* ---------- Magnetic buttons + 3D tilt + cursor (fine pointers only) ---------- */
if (fine && !reduce) {
  // Magnetic
  ...
  if (cursor && ring) {
    ...
    requestAnimationFrame(ringRaf);
    document.querySelectorAll("a, button, [data-tilt], input, select, textarea, label").forEach((el) => {
